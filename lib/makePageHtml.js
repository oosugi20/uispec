 ...
 */
const makePageHtml = function ( contents, templatePath ) {
...
const makeSvgCanvasSrc = function ( screenUrl, screenFullPath ) {
	...
	const reg    = /highlight=\[(\[[0-9]+,[0-9]+,[0-9]+,[0-9]+\],?)+\]/;
	const match  = reg.exec( screenUrl );
	const coords = match ? JSON.parse( match[ 0 ].replace( /highlight=/, '' ) ) : [];
