...
uispec.init = function (options) {
  ...
  const config = (helpers.isExistFile('.uispecconfig')) ?
...
uispec.generate = function () {
  this.clean();
  const allSrcFullFilePaths = getAllSrcFilePaths({ onlyMd: false });

  _.forEach(allSrcFullFilePaths, (fileFullPath) => {
    let data;
    const isMd = /\.md/.test(fileFullPath);
    const destPath = (isMd) ?
      fileFullPath.replace(this.srcDir, this.destDir).replace(/\.md$/, '.html') :
      fileFullPath.replace(this.srcDir, this.destDir);
  ...
  // TODO path.sep()
    ...
    if (isMd) {
      ...
      renderHtml(fileFullPath, fileDir, (toRoot) ? toRoot + '/' : '').then((data) => {
...
const FileListDataCollection = require('./FileListDataCollection');
function renderFilelist () {
  const allMdFullFilePaths = getAllSrcFilePaths({ onlyMd: true });
  const filelistData = {};
  //----------
  //const allMdData = _.map(allMdFullFilePaths, (mdFileFullPath) => {
  //  return helpers.readMarkdownFile(mdFileFullPath);
  //});
  //const collection = new FileListDataCollection(allMdData);
  //----------
  filelistData.items = _.map(allMdFullFilePaths, (mdFileFullPath) => {
    return getFilelistData(mdFileFullPath);
  });

  filelistData.items.sort((a, b) => {
    const regIndex = /index\.md/;

    if (a.srcFullDir < b.srcFullDir) {
      return -1;
    } else if (a.srcFullDir > b.srcFullDir) {
      return 1;
    }
...
async function renderHtml (mdFullPath, screenReqDir, toRoot) {
  const mdData = helpers.readMarkdownFile(mdFullPath);

  if (mdData.update) {
    mdData.updateHtml = marked(mdData.update).replace(/<p>|<\/p>/g, '');
  }
...
uispec.serve = function (options) {
  ...
  const opts = _.assign({}, {
    srcDir: uispec.srcDir,
    serveDir: uispec.serveDir,
  }, options);
  ...
  return function (req, res, next) {
    ...
    if (myPath.isImg()) {
      const sendPath = path.join(
        process.cwd(),
        opts.srcDir,
        myPath.reqOriginalPath.replace(opts.serveDir, '')
      );
    ...
    const toRoot = path.relative(myPath.reqPath.dir, opts.serveDir);
...
function getFilelistData (mdFileFullPath) {
  const mdData = helpers.readMarkdownFile(mdFileFullPath);

  const isIndex = /index\.md$/.test(mdData.srcFullPath);


  const srcRootFullDir = path.join(process.cwd(), uispec.srcDir);
...
class MyPath {
  ...
  isExistFile() {
    return helpers.isExistFile(this.targetMdPathAbsolute);
  }
}

function getAllSrcFilePaths ({ onlyMd }) {
  const results = [];
  const fullRootDir = path.join(process.cwd(), uispec.srcDir);
  getAllFilePaths(fullRootDir);
  return results;
